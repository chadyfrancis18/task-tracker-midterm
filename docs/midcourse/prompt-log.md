...
- Refined Prompt: "Write a FastAPI backend validation function for due_date using the YYYY-MM-DD ISO format within our existing Task Tracker, returning a 422 status code on validation error, and compute an overdue boolean value dynamically."
...
- Refined Prompt: "Extend the FastAPI GET /tasks route in the Task Tracker to support an optional query parameter overdue=true that filters the task list returning only items where overdue is true."
...
- Refined Prompt: "Implement robust backend handling in the FastAPI Task Tracker for comma-separated tag inputs, ensuring tags are stripped of whitespace, empty strings are rejected, and unique lists are stored."
...
- Refined Prompt: "Update the FastAPI GET /tasks route in the Task Tracker to support an optional tag query parameter that filters tasks containing the specified tag string."
